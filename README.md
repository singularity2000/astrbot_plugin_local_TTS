...
- **缓存管理**：支持手动清理，或按保留时间、总大小和文件数量自动清理语音缓存，节约存储空间。
...
该命令用于清空语音缓存，释放磁盘空间；正在使用的文件会暂时跳过并提示。
...
- **缓存管理**
  - **缓存最长保留时间（小时）**：超过保留时间的文件会被清理，按文件最后修改时间计算，支持小数。
  - **缓存总大小上限（MB）**：所有缓存文件的总容量上限，支持小数；1 MB = 1024 × 1024 字节。
  - **缓存文件数量上限（个）**：最多保留的文件数，填写正整数。
  - 每项**留空表示不限制**，三项均留空则关闭自动清理。填写时须为正数，不接受 `0`、负数或无效内容；配置错误时暂停自动清理。
  - 任意一项超限都会触发清理，优先删除最旧文件，直到满足全部已填写的限制。启动后、发送完成后及每分钟检查；正在使用的文件暂不删除，因此可能短暂超限。
  - 配置在插件重载或重启后生效；`TTS清理` 仍可手动清空缓存。
